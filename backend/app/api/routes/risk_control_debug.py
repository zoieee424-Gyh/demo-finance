"""Debug endpoint for RiskControlDeepAgent."""
...
from app.schemas.risk_control_debug import RiskControlDebugResponse
...
router = APIRouter(tags=["risk-control-debug"])


@router.post("/debug/risk-control", response_model=RiskControlDebugResponse)
def debug_risk_control(payload: ConsultationRequest) -> RiskControlDebugResponse:
    planner = RagPlanner()
    ...
    planned_queries = planner.build_queries(payload, "risk_control")
    ...
    return RiskControlDebugResponse(
...
def _get_arch(agent) -> AgentArchitectureDebug:
    if hasattr(agent, "debug_info"):
        ...
        traces = [
            ToolCallTraceItem(
                tool_id=t.get("tool_id", ""), name_cn=t.get("name_cn", ""),
        ...
        return AgentArchitectureDebug(
            ...
            tool_count=info.get("tool_count", 0), tool_traces=traces,
        )
