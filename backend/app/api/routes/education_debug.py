"""Debug endpoint for FinancialEducationDeepAgent."""
...
from app.schemas.education_debug import EducationDebugResponse
...
router = APIRouter(tags=["education-debug"])


@router.post("/debug/education", response_model=EducationDebugResponse)
def debug_education(payload: ConsultationRequest) -> EducationDebugResponse:
    """Run the financial education agent directly for API-level debugging.

    This endpoint bypasses intent routing: every request is handled
    as education so the fifth agent can be tested independently.
    """
    planner = RagPlanner()
    ...
    planned_queries = planner.build_queries(payload, "education")
